...
function month_sel($create_time){
	$y = date("Y-m",$create_time['create_time']);
	$z = date("Y-m",time());
	$c = strtotime("{$y}-01");
	$b = strtotime("{$z}-01");
	$a = 0;
	...
	while($c <= $b){
		$c = strtotime("+{$a} months",strtotime("{$y}-01"));
		$d = strtotime("+1 months",$c);
		$data[]="create_time>={$c} and create_time<{$d}";
		$time[]="{$c}";
		$a++;
	}
...
function id_card($id_card){
	$str1   = substr($id_card,0,6);
	$str2   = substr($id_card,-5);
	$id_card = $str1.'*******'.$str2;
	return $id_card;
}

/**
 * @return 返回本月的第一天及最后一天
 * 获取当前月的时间
 */
function now_month(){
	$BeginDate=date('Y-m-01', strtotime(date("Y-m-d")));
	$end_time  = date('Y-m-d', strtotime("$BeginDate +1 month -1 day"));
	$time['start_time'] = $BeginDate;
...
function is_examine(){
	...
	$user_ex = D('user')->where(array('id'=>$user_id))->field('examine_status')->find();
	return $user_ex['examine_status'];
 ...
 */
function send_info_fiel($mobile,$content){
	vendor ("Cxsms.Cxsms");
	$options = array(
		'userid'  =>'1167',
		'account' =>'18781176753',
