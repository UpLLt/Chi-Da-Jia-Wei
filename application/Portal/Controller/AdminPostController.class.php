...
class AdminPostController extends AdminbaseController {
	...
	function add_post(){
		if (IS_POST) {
			...
			$_POST['smeta']['thumb'] = $_POST['smeta']['thumb'];
			...
			foreach ($_POST['term'] as $mterm){
				$article['term_id']   = $mterm;
			}
	...
	function delete(){
		if(isset($_GET['tid'])){
			...
			$data['status']=0;
			$sql = M('term_relationships')->where("tid='{$tid}'")->find();
			M('posts')->where("id='{$sql['object_id']}'")->save($data);
